...
fn part1(input: &str) -> String {
    ...
    let squares = find_energized_squares(&map);
    let output = square_count(&squares);

    output.to_string()
}

fn find_energized_squares(map: &Map) -> Vec<String> {
    let mut history: StepHistory = HashMap::new();
    let mut to_visit = Vec::<Step>::new();

    to_visit.push(((0, 0), Direction::R));
